...
It is said that SQLAlchemy doesn’t reflect it **on any backend**. But it’s strange that it works for PostgreSQL and it works when we pass `copy_from` argument with `__table_args__`  tuple that includes the constraints.
