...
As explained before, batch operations are used for SQLite to not lose any data. Due to that, it needs further actions to run **--sql** command. These actions were tried out and cause
...
| For example, let's check a small migration script named *2020-06-30_b311affac706_update_nullability_of_columns_of_.py*.
