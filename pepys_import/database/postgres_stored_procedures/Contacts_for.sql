...
--Version: v0.18
	...
	ui_filter_input as
		...
		(
			...
			where
				...
				((select source_map from ui_filter_input) is null OR (con.source_id, con.sensor_id) in (select source_id, sensor_id from sensor_source_maps))
		),
		filtered_limits as
		(select
	...
	select filtered_contacts.contact_id, filtered_contacts.time, filtered_sensors.sensor_name, filtered_sensors.platform_name,
			filtered_sensors.platformtype_name, filtered_sensors.nationality_name,
