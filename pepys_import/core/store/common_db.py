...
class ContactMixin:
    ...
    def sla(self, sla):
        ...
        try:
            if not sla.check(""):
                raise ValueError("SLA must be a Quantity with a dimensionality of '' (ie. nothing)")
            if not (sla.units == unit_registry.degree or sla.units == unit_registry.radian):
                raise ValueError("SLA must be a Quantity with angular units (degree or radian)")
        except AttributeError:
